...
export default function ContasReceber() {
  const { pode } = useAuth();
  const podeOperar = pode('/financeiro/receber', 'EDITAR');
  ...
  const cancelar = async (c: Conta) => {
    ...
    try {
      await financeiroApi.cancelarReceber(c.id, 'Cancelado pela tela de Contas a Receber.');
      ...
      <div className="flex-1 overflow-auto p-6 space-y-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar por cliente, descrição ou nº..." className="bg-[#101216] border border-[#23262F] rounded-lg pl-8 pr-3 py-2 text-sm text-[#F7F8FA] w-80 focus:outline-none focus:border-[#01B8FA]/60" />
          </div>
        ...
        <div className="bg-[#101216] rounded-2xl border border-[#23262F] overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-[#0C0D10] text-xs text-[#8A90A0] border-b border-[#23262F]">
              <tr>
                {['Cliente / Descrição', 'Nº', 'Vencimento', 'Valor', 'Recebido', 'Em aberto', 'Status', ''].map((h, i) => (
...
function ModalBaixa({ conta, onClose, onDone }: { conta: Conta; onClose: () => void; onDone: () => void }) {
  ...
  const [contasFin, setContasFin] = useState<{ id: string; nome: string; saldoAtual: number; padrao: boolean }[]>([]);
  ...
  const confirmar = async () => {
    ...
    setSalvando(true);
    try {
      await financeiroApi.baixarReceber(conta.id, { valor: v, formaPagamento: forma || undefined, contaId: contaId || undefined });
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 animate-backdrop" onClick={onClose}>
      <div className="relative w-full max-w-sm bg-[#101216] border border-[#23262F] rounded-2xl shadow-[0_24px_80px_-12px_rgba(0,0,0,0.7)] p-5 animate-modal" onClick={e => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="font-bold text-[#F7F8FA]">Baixar recebimento</h2>
        ...
        <label className="block mb-3">
          <span className="text-xs text-[#8A90A0]">Valor recebido</span>
        ...
        <label className="block mb-4">
          <span className="text-xs text-[#8A90A0]">Conta de destino (tesouraria)</span>
...
function ModalNovo({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const confirmar = async () => {
    ...
    setSalvando(true);
    try {
      await financeiroApi.criarReceber({
  ...
  return createPortal((
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 animate-backdrop" onClick={onClose}>
      <div className="relative w-full max-w-sm bg-[#101216] border border-[#23262F] rounded-2xl shadow-[0_24px_80px_-12px_rgba(0,0,0,0.7)] p-5 animate-modal" onClick={e => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-4">
          <h2 className="font-bold text-[#F7F8FA]">Novo título a receber</h2>
          <button onClick={onClose} className="h-8 w-8 rounded-lg hover:bg-[#16181F] text-[#8A90A0] flex items-center justify-center"><X className="h-4 w-4" /></button>
        </div>
        ...
        <button onClick={confirmar} disabled={salvando} className="mt-4 w-full flex items-center justify-center gap-2 bg-[#01B8FA] hover:bg-[#22D3EE] disabled:opacity-40 text-[#04121A] font-bold py-2.5 rounded-lg">
