...
const STATUS_META: Record<string, { label: string; cls: string; cor: string; icon: any }> = {
  TRIAL:     { label: 'Em teste',   cls: 'text-[#E8A317]', cor: '#E8A317', icon: Clock },
  ATIVA:     { label: 'Ativas',     cls: 'text-[#2DD4A7]', cor: '#2DD4A7', icon: CheckCircle2 },
  SUSPENSA:  { label: 'Suspensas',  cls: 'text-[#F59E0B]', cor: '#F59E0B', icon: PauseCircle },
  CANCELADA: { label: 'Canceladas', cls: 'text-[#FF6B7A]', cor: '#FF6B7A', icon: Ban },
};
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="bg-[#101216] border-b border-[#23262F] px-5 py-3 flex items-center gap-3 shrink-0">
        ...
        <div>
          <h1 className="text-[15px] font-bold text-[#F7F8FA] leading-tight">Visão Geral da Plataforma</h1>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <section className="bg-[#101216] border border-[#23262F] rounded-xl p-4">
              <h3 className="text-sm font-bold text-[#F7F8FA] mb-3">Assinaturas por status</h3>
            ...
            <section className="bg-[#101216] border border-[#23262F] rounded-xl p-4 lg:col-span-2">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-[#F7F8FA]">Maiores receitas</h3>
                <button
...
function Kpi({ icon: Icon, cor, titulo, valor, legenda, destaque }: {
  ...
  return (
    <div className={`bg-[#101216] border rounded-xl p-3.5 ${destaque ? 'border-[#2DD4A7]/40' : 'border-[#23262F]'}`}>
      ...
      <p className="text-xl font-bold text-[#F7F8FA] leading-tight">{valor}</p>
...
function Atalho({ icon: Icon, titulo, texto, onClick }: { icon: any; titulo: string; texto: string; onClick: () => void }) {
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <p className="text-sm font-bold text-[#F7F8FA] group-hover:text-[#01B8FA] flex items-center gap-1">
