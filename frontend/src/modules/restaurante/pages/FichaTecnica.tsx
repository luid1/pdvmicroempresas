...
const corCmv = (v: number) =>
  v <= 30 ? 'text-[#0b7d4e]' : v <= 38 ? 'text-[#a9760a]' : 'text-[#c3352b]';
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-[#E5E7EB] px-5 py-3 shrink-0 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-[15px] font-bold text-[#16171D] leading-tight">Fichas Técnicas</h1>
      ...
      <div className="flex-1 overflow-hidden flex">
        {/* Lista de fichas */}
        <div className="w-full sm:w-[340px] shrink-0 border-r border-[#E5E7EB] overflow-auto bg-[#FAFAF8]">
          {visiveis.map((f) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-[#EEEDE9] transition-colors ${
                  ativo ? 'bg-[#01B8FA]/[0.06] border-l-2 border-l-[#01B8FA]' : 'hover:bg-white border-l-2 border-l-transparent'
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <p className="font-bold text-[#16171D] text-sm truncate">{f.nome}</p>
                  <span className={`text-[11px] font-bold shrink-0 ${corCmv(c)}`}>{pct(c)}</span>
                </div>
                <div className="flex items-center justify-between mt-1 text-[11px] text-[#8B8D98]">
          ...
          {visiveis.length === 0 && (
            <p className="text-center py-12 text-sm text-[#A0A2AD]">Nenhuma ficha.</p>
          )}
        </div>
        ...
        <div className="flex-1 overflow-auto p-5 hidden sm:block">
          {aberta ? <DetalheFicha ficha={aberta} onPreco={setPreco} onRemover={removerInsumo} /> : (
            <div className="h-full flex items-center justify-center text-[#A0A2AD] text-sm">
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-black text-[#16171D]">{ficha.nome}</h2>
          <p className="text-[12px] text-[#8B8D98] flex items-center gap-1 mt-0.5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
        <KpiFicha icon={DollarSign} titulo="Custo do prato" valor={brl(custo)} cor="text-[#16171D]" />
        <KpiFicha icon={TrendingUp} titulo="Preço de venda" valor={brl(ficha.precoVenda)} cor="text-[#0678a0]" />
        <KpiFicha icon={Percent} titulo="CMV" valor={pct(c)} cor={corCmv(c)} />
        <KpiFicha icon={TrendingUp} titulo="Margem (R$)" valor={brl(m)} cor={m > 0 ? 'text-[#0b7d4e]' : 'text-[#c3352b]'} />
      </div>

      {/* Preço editável */}
      <div className="mt-4 flex items-center gap-3 rounded-xl border border-[#E7E5DF] bg-[#F6F5F2] px-4 py-3">
        <label className="text-[12px] text-[#5B5D69] font-medium">Preço de venda</label>
        <div className="flex items-center gap-1">
          <span className="text-[#8B8D98] text-sm">R$</span>
          <input
            type="number"
            step="0.10"
            value={ficha.precoVenda}
            onChange={(e) => onPreco(ficha.id, Number(e.target.value) || 0)}
            className="w-24 text-sm font-bold rounded-lg px-2 py-1.5 text-[#16171D] bg-white border border-[#E7E5DF] focus:outline-none focus:border-[#01B8FA]/50"
          />
        </div>
        <p className="text-[11px] text-[#A0A2AD] ml-auto">
      ...
      {/* Insumos */}
      <div className="mt-5">
        <h3 className="text-xs font-bold uppercase tracking-wide text-[#8B8D98] mb-2">Insumos da ficha</h3>
        <div className="rounded-xl border border-[#E7E5DF] overflow-hidden">
          <table className="w-full text-[12px]">
            <thead className="bg-[#F6F5F2] text-[#8B8D98]">
              <tr>
            ...
            <tbody>
              {ficha.insumos.map((i) => (
                <tr key={i.id} className="border-t border-[#E7E5DF] hover:bg-[#FAFAF8]">
                  <td className="px-3 py-2 text-[#16171D] font-medium">{i.nome}</td>
                  <td className="px-3 py-2 text-right text-[#5B5D69]">{i.qtd} {i.unidade}</td>
                  <td className="px-3 py-2 text-right text-[#8B8D98]">{brl(i.custoUnit)}/{i.unidade}</td>
                  <td className="px-3 py-2 text-right font-bold text-[#16171D]">{brl(i.qtd * i.custoUnit)}</td>
                  <td className="px-3 py-2 text-right">
                    <button
                      onClick={() => onRemover(ficha.id, i.id)}
                      className="text-[#C7C9D2] hover:text-[#c3352b] transition-colors"
              ...
              {ficha.insumos.length === 0 && (
                <tr><td colSpan={5} className="text-center py-6 text-[#A0A2AD]">Sem insumos — adicione para calcular o custo.</td></tr>
              )}
            </tbody>
            <tfoot>
              <tr className="border-t border-[#E7E5DF] bg-[#F6F5F2] font-bold">
                <td className="px-3 py-2 text-[#5B5D69]" colSpan={3}>Custo total do prato</td>
                <td className="px-3 py-2 text-right text-[#16171D]">{brl(custo)}</td>
                <td />
              </tr>
            </tfoot>
          </table>
        </div>
        <button className="mt-3 flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border border-dashed border-[#01B8FA]/40 text-[#0678a0] hover:bg-[#01B8FA]/[0.06] transition-colors">
...
function KpiFicha({
  ...
  return (
    <div className="rounded-xl border border-[#E7E5DF] bg-white px-3 py-2.5">
