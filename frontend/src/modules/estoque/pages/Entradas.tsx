...
function ModalEntrada({ onClose, onSalvo, filialId, ocId }: { onClose: () => void; onSalvo: () => void; filialId?: string; ocId?: string }) {
  ...
  return (
    <Modal titulo={ocNumero ? `Receber OC #${ocNumero} — Entrada de Mercadoria` : 'Nova Entrada de Mercadoria'} onClose={onClose} onSalvar={salvar} salvando={salvando} salvarLabel="Dar entrada" wide>
      {/* Upload XML */}
      <label className="flex items-center gap-3 bg-[#FBFAF7] border border-dashed border-[#E7E5DF] rounded-lg px-4 py-3 cursor-pointer hover:border-[#E8A317]">
        <Upload className="h-5 w-5 text-[#a9760a]" />
        <div className="flex-1"><p className="text-sm font-semibold text-[#5B5D69]">Importar XML da NF-e do fornecedor</p><p className="text-xs text-slate-500">Preenche os itens automaticamente (você confere o vínculo dos produtos).</p></div>
        <FileCode className="h-4 w-4 text-slate-500" />
        <input type="file" accept=".xml,text/xml" className="hidden" onChange={e => e.target.files?.[0] && onXml(e.target.files[0])} />
      </label>
      {aviso && <p className="text-xs text-[#a9760a] bg-[#E8A317]/12 px-3 py-2 rounded-lg">{aviso}</p>}
        ...
        {itens.map((it, i) => {
          ...
          return (
            <div key={i} className="border border-[#E7E5DF] rounded-lg p-3 bg-[#F6F5F2]">
              ...
              <div className="text-right text-[11px] text-slate-500 mt-1.5">Subtotal: <b className="text-[#8B8D98]">{R$((Number(it.quantidade) || 0) * (Number(it.valorUnitario) || 0))}</b></div>
            </div>
          );
        })}
      </div>
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-[#8B8D98]">Total da nota: <b className="text-[#16171D]">{R$(totalNota)}</b></span>
      </div>

      <Secao titulo="Financeiro" />
      <div className="flex flex-wrap items-center gap-4">
        <label className="flex items-center gap-2 text-sm text-[#5B5D69]"><input type="checkbox" checked={gerarCP} onChange={e => setGerarCP(e.target.checked)} className="accent-amber-500 h-4 w-4" /> Gerar Contas a Pagar</label>
