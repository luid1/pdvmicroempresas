...
export default function FiscalConfiguracao() {
  ...
  const validar = async () => {
    ...
    try {
      ...
      setConfig(data); toast(data.ok ? 'Requisitos mínimos conferidos.' : 'Ainda existem pendências fiscais.', data.ok ? 'success' : 'info');
  ...
  return <div className="min-h-full bg-[#F7F7F8] p-4 sm:p-5"><div className="mx-auto max-w-6xl space-y-4 pb-24">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div><h1 className="text-xl font-semibold text-[#202123]">Central SEFAZ</h1><p className="mt-1 text-xs text-slate-500">Configuração tributária e credenciais separadas por filial e por provedor.</p></div>
