...
export class IaService {
  ...
  async comando(
    ...
    const podeLancar = podeVerAlguma(usuario.telas, usuario.role, TELAS_TESOURARIA);
    ...
    try {
      const cru = await this.gemini.gerar({
        ...
        prompt: this.promptClassificacao(q, podeLancar, historico),
        json: true,
        maxTokens: 300,
        temperatura: 0,
      });
      intencao = this.parseJson(cru);
    } catch (e) {
      this.log.warn(`Falha ao classificar comando, tratando como pergunta: ${e}`);
    }

    const tipo = intencao?.tipo;

    // 2) Ação de lançamento
    if (tipo === 'acao' && ACOES_IA[intencao?.acao as TipoAcao]) {
      if (!podeLancar) {
      ...
      const acao = intencao.acao as TipoAcao;
  ...
  private promptClassificacao(
    ...
    return [
      ...
      '  "acao": "lancar-gasto" | "lancar-entrada" | null,',
      '  "campos": { "valor": number|null, "descricao": string|null, "categoria": string|null, "data": "YYYY-MM-DD"|null },',
